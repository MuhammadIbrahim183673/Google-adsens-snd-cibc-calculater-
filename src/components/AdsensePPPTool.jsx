...
export default function AdsensePPPTool() {
  ...
  const handleEngage = async () => {
    ...
    const newEntry = { date: new Date().toLocaleDateString(), earned };
    const newHistory = [...history, newEntry];

    // Auto-assign tier
    const newTier =
      updatedRewards >= 5000 ? 'Partner' :
      updatedRewards >= 1000 ? 'Builder' : 'Starter';
  ...
  if (!user) {
    return (
      <div className="grid gap-6 p-6 max-w-md mx-auto">
        <h1 className="text-2xl font-bold text-center">Login to Your Dashboard</h1>
        <Card>
  ...
  return (
    <div className="grid gap-6 p-6 max-w-3xl mx-auto">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Welcome, {profile.displayName || user.email}</h1>
      ...
      <Card>
        <CardContent className="grid gap-4 py-6">
          ...
          <div className="bg-green-100 rounded-xl p-4 text-green-800 font-medium flex items-center gap-2">
            ...
            Estimated Daily Earnings: <span className="text-lg font-bold">${earnings}</span>
          </div>

          <div className="bg-blue-100 rounded-xl p-4 text-blue-800 font-medium flex items-center gap-2">
            <Coins className="w-5 h-5" />
            PPP Rewards Earned: <span className="text-lg font-bold">{rewards} XIBC</span>
          </div>
          ...
          <ul className="grid gap-2">
            {history.map((entry, index) => (
              <li key={index} className="text-sm text-gray-700">
                {entry.date}: Earned <span className="font-bold">{entry.earned} XIBC</span>
              </li>
            ))}
            {history.length === 0 && (
